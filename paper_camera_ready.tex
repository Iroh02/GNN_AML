...
runs highlight the limitations of single-seed results. We further show that
one of the three wallet-sharing relation types is entirely absent from the
training portion of the standard benchmark split, the most visible symptom of
a temporal distribution shift that---rather than model capacity---accounts
for most of the observed validation--test gap. An explanation module provides
...
detectable benefit from the gating; (iii) evidence that seed-to-seed spread on this benchmark is
comparable to the differences between architectures, so single-seed
...
the official HBTBD training split---a previously unreported symptom of a
temporal distribution shift that accounts for most of the validation--test
gap. An explanation module
...
($\pm 0.35$). Attribution requires care: the resplit both makes M2 available
in training \emph{and} equalises the train/test feature distributions, so it
cannot separate the two. The feature-only MLP settles the question---it uses
no metapaths yet shows an even larger gap ($34.7 \pm 2.2$ points), so the
dominant cause is temporal distribution shift in the features themselves, of
which the M2 asymmetry is the most visible structural symptom. Either way
the practical conclusion for users of this benchmark stands: the official
split severely understates deployment-style performance, and random-split
evaluations (as in the dataset's original paper) severely overstate it.
...
passing could reach. We consider the two benchmark findings---the temporal
shift in HBTBD's official split and the strength of capacity-matched
feature-only baselines---the most useful contributions, since they affect
anyone evaluating GNNs on these datasets.
...
architecture conclusions. Our attribution of
the validation--test gap to temporal feature shift rests on the MLP control;
a resplit with M2 removed would isolate M2's marginal contribution directly
and is left for future work. Both benchmarks are academic-scale (203K and
...
transferable result is a property of the benchmark rather than any model:
HBTBD's official split carries a severe temporal distribution shift---most
visibly, the M2 metapath is absent from training---that accounts for most of
