...
heterogeneous GNN that approximates the role of the wallet address connecting
pairs of transactions and weights each wallet-sharing relationship using
priors informed by financial-crime investigation. Across ten random seeds
...
domain-adapted variant ($+1.3$ to $+2.5$ ROC-AUC points, $p\le0.03$), while a
naive heterogeneous GNN ranks \emph{worse} than the feature-only model.
...
graph structure; (ii) RiskMAGNN, a domain-adapted variant of the
metapath-aggregated GNN (MAGNN)~\cite{fu2020} combining a translation-based
approximation of the shared wallet address~\cite{bordes2013} with attention
over relation types initialised from forensic priors, with a component
ablation; (iii) evidence that seed-to-seed spread on this benchmark is
...
\textbf{Translation-based metapath encoding.} MAGNN encodes every node along
a metapath instance with an intra-metapath instance encoder (mean, linear, or
relational rotation)~\cite{fu2020}, which requires the identity and features
of the intermediate node. HBTBD, however, releases only pre-aggregated
transaction-to-transaction adjacencies in which the identity of the shared
wallet address is not preserved, so MAGNN's instance encoding cannot be
applied to this data as shipped. Because for AML that address carries
independent forensic signal---an address recurring between illicit
transactions is itself evidence---we approximate the missing intermediate
with a TransE-style encoding~\cite{bordes2013}: node features are projected
to a shared $d$-dimensional space $h'$, and for instance
...
\end{equation}
where $r_1, r_2 \in \mathbb{R}^d$ are learned relation vectors, $\odot$ is
the Hadamard product, $\oplus$ is concatenation, and $t$ is proxied by the
mean of $v$'s metapath neighbours. This adapts MAGNN-style instance encoding
to a data release lacking intermediate node features; it addresses a
limitation of the dataset, not of MAGNN.
...
free signal: it pays off only when the aggregation carries domain structure,
and its measurable benefit is a modest, reliable ranking gain rather than
average precision.
...
without engineered aggregates the comparison could differ. Our attribution of
...
only with domain-adapted aggregation ($+1.3$ to $+2.5$ ROC-AUC points,
$p\le0.03$, versus both a feature-only MLP and a naive heterogeneous GNN),
